...
class ChargesPanel(QWidget):
    def __init__(self, win) -> None:
        ...
        self.win = win
        sub = QLabel(
            "Flujo de switch/adquirente ISO 8583. Config por variables del proyecto "
        ...
        self._amount = QLineEdit("500")
        self._amount.setFixedWidth(100)
        self._dry = QCheckBox("dry-run")
        self._dry.setChecked(True)
        self._signon = QCheckBox("sign-on antes")
        self._signon.setChecked(True)
        row = QHBoxLayout()
        row.addWidget(QLabel("Monto (centavos)"))
        row.addWidget(self._amount)
        row.addWidget(self._dry)
        row.addWidget(self._signon)
        row.addStretch(1)

        b_so = QPushButton("Sign-on (0800)")
        b_so.clicked.connect(lambda: self._go("signon"))
        b_pur = QPushButton("Cobrar (0200)")
        b_pur.clicked.connect(lambda: self._go("purchase"))
        b_rev = QPushButton("Reverso (0400)")
        b_rev.clicked.connect(lambda: self._go("reversal"))
        brow = QHBoxLayout()
    ...
    def _go(self, kind: str) -> None:
        ...
        self.win.run_switch_flow(
            kind,
            amount=self._amount_cents(),
            dry_run=self._dry.isChecked(),
            sign_on=self._signon.isChecked(),
        )
