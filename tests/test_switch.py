...
_BC = {
    ...
    "expiry": "2909", "aid": "A0000000031010", "aip": "2000",
    "atc": "002F", "arqc": "D6F5B2E0E50B0F9C", "iad": "06011203A02000", "un": "ED999B69",
}
...
def test_config_from_vars():
    data = {"switch_host": "200.53.144.37", "switch_port": "2347",
            "switch_tls": "1", "tpdu": "6000000000", "terminal_id": "14400757",
            "merchant_id": "8647574", "mcc": "5541"}
...
def test_build_purchase_has_f55_and_terminal():
    ...
    assert tlv.parse(fields[55]).find("9F26").value == from_hex("D6F5B2E0E50B0F9C")  # ARQC
...
def test_run_purchase_against_mock_switch():
    ...
    approved_0210 = iso8583.build("0210", {
        4: iso8583.bcd_amount(500), 11: iso8583.bcd("000001"),
        39: b"00", 41: b"14400757"})
...
def test_all_templates_scaffold_and_load(tmp_path):
    ...
    registry.clear()
    loaded, errors = registry.load_plugins(SimpleNamespace(pocs_dir=pocs, path=tmp_path))
