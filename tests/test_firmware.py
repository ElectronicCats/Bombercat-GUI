...
def test_parse_fw_names():
    ...
    assert bt.parse_fw_names(sample) == ["NFCGate", "magspoof", "ESP32SerialPassthroughFlash"]
...
def test_firmware_tab_list_and_flash(monkeypatch):
    ...
    monkeypatch.setattr(bt, "flash_capture",
                        lambda name, **k: SimpleNamespace(returncode=0,
                                                          stdout=f"flashed {name}", stderr=""))
...
def test_compile_and_flash_uses_upload_not_uf2_flasher(monkeypatch):
    ...
    monkeypatch.setattr(ard, "latest_uf2", lambda *a, **k: None)   # este core no genera .uf2
