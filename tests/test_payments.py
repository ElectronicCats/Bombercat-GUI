...
def test_iso8583_roundtrip():
    fields = {
        2: iso8583.bcd("4189143370041827"),
        3: from_hex("000000"),
...
def test_translate_response_code():
    view = iso8583.translate(
        iso8583.build("0210", {4: iso8583.bcd_amount(1500), 39: b"51", 41: b"14400757"})
    )
...
def test_cryptogram_gap_and_clean():
    cards = [
        EmvCard(pan="4189143370041827", atc="002F", arqc="AA", un="11"),
...
def test_cryptogram_un_reuse_medium():
    cards = [
        EmvCard(pan="5", atc="0001", arqc="A1", un="CAFE"),
        EmvCard(pan="5", atc="0002", arqc="A2", un="CAFE"),
    ]
    rep = cryptogram.analyze(cards)
    assert any(
        o.kind == "un_reuse" and o.severity == "medium" for o in rep.observations
    )
