...
def test_explorer_assign_and_save(win, tmp_path):
    ...
    ep._savename.setText("cap1")
    ep._save()
    assert [p.name for p in store.list_captures(store.active_project())] == [
        "cap1.json"
    ]
...
def test_fuzz_emv_source_uses_capture(win):
    ...
    fz._ndef_src.setCurrentIndex(
        fz._ndef_src.findData("emv")
    )  # → _reload_captures(canned+RAM)
...
def test_fuzz_emv_ram_sources(win):
    ...
    fz._ndef_src.setCurrentIndex(
        fz._ndef_src.findData("emv")
    )  # puebla el picker (canned+RAM+capturas)
    # RAM del BomberCat
...
def test_poc_ide_create_edit_save(win):
    ...
    p._save()
    assert (
        (store.active_project().pocs_dir / "my-poc.py")
        .read_text()
        .startswith("# editado")
    )
