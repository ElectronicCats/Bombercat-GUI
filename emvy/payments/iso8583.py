...
# Mensaje ISO 8583
# ---------------------------------------------------------------------------
def build(mti: str, fields: dict[int, bytes], spec: dict[int, FieldSpec] = DEFAULT_SPEC) -> bytes:
...
def parse(data: bytes, spec: dict[int, FieldSpec] = DEFAULT_SPEC) -> tuple[str, dict[int, bytes]]:
...
def build_network_0800(stan: str | None = None, info: str = "0001") -> bytes:
    ...
    fields = {7: bcd(now.strftime("%m%d%H%M%S")), 11: bcd(stan[:6].zfill(6)),
              70: from_hex(info)}
...
def response_meaning(code: str | None) -> str:
    ...
    if not code:
        return "(sin DE 39)"
    return _RESPONSE_CODES.get(code, f"código {code} (no estándar / específico del switch)")


# ---------------------------------------------------------------------------
# Ensamblado del campo 55 (ICC / EMV) desde un EmvCard
# ---------------------------------------------------------------------------
def emv_icc(card: EmvCard, *, amount_cents: int | None = None,
            terminal: dict[str, str] | None = None) -> bytes:
...
_MTI_VERSION = {
    "0": "ISO 8583:1987", "1": "ISO 8583:1993", "2": "ISO 8583:2003",
    "8": "reservado nacional", "9": "reservado privado",
}
_MTI_CLASS = {
    "1": "autenticación", "2": "autorización", "3": "archivo (file)",
    "4": "reverso/devolución", "5": "reconciliación", "6": "administrativo",
    "7": "fee/notificación", "8": "gestión de red", "9": "reservado",
}
...
_MTI_ORIGIN = {
    "0": "adquirente", "1": "repetición del adquirente",
    "2": "emisor", "3": "repetición del emisor",
    "4": "otro", "5": "repetición de otro",
}

_PROCESSING_CODES = {
    "00": "compra de bienes/servicios", "01": "retiro de efectivo",
    "09": "compra con cashback", "17": "retiro (cuota)", "20": "devolución (refund)",
    "21": "depósito", "28": "pago", "30": "consulta de saldo",
    "31": "consulta", "40": "transferencia", "50": "pago de servicios",
    "72": "recarga de tiempo aire",
}
_RESPONSE_CODES = {
    "00": "aprobada", "01": "referir al emisor", "03": "comercio inválido",
    "04": "retener tarjeta", "05": "no honrar (do not honor)",
    "12": "transacción inválida", "13": "monto inválido",
    "14": "tarjeta inexistente", "30": "error de formato",
    "41": "tarjeta perdida", "43": "tarjeta robada", "51": "fondos insuficientes",
    "54": "tarjeta expirada", "55": "PIN incorrecto",
    "57": "no permitida a la tarjeta", "58": "no permitida al terminal",
    "61": "excede límite de retiro", "62": "tarjeta restringida",
    "65": "excede frecuencia de retiro", "75": "excede intentos de PIN",
    "91": "emisor no disponible", "96": "malfuncionamiento del sistema",
}
_CURRENCY_CODES = {
    "0484": "MXN", "0840": "USD", "0978": "EUR", "0826": "GBP", "0392": "JPY",
    "0124": "CAD", "0032": "ARS", "0986": "BRL", "0170": "COP", "0604": "PEN",
    "0152": "CLP", "0356": "INR", "0036": "AUD",
}
_POS_ENTRY = {
    "01": "manual", "02": "banda magnética", "05": "chip (ICC)",
    "07": "contactless (chip)", "80": "fallback a banda", "90": "banda (pista completa)",
    "91": "contactless (banda)", "95": "chip sin CVV confiable",
}
...
class Iso8583View:
    ...
    def text(self, color=None) -> str:
        ...
        for f in self.des:
            head = (f"  DE {f.de:>3}  {color(f.name, 'cyan')}  "
