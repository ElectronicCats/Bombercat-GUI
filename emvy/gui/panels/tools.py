...
class _FlagsTool(QWidget):
    def __init__(self, win) -> None:
        ...
        search = QPushButton("Buscar patrón")
        search.clicked.connect(self._search)
        default = QPushButton("Patrones por defecto")
        default.clicked.connect(self._default)
        row = QHBoxLayout()
        row.addWidget(self._pat, 1)
    ...
    def _dump(self):
        ...
        if d is None:
            self._log.appendPlainText(
                "No hay captura. Ve a Explorador → Capturar primero."
            )
        return d
    ...
    def _search(self) -> None:
        ...
        if d:
            self._render(
                search_regex(d.all_blobs(), self._pat.text() or r"flag\{[^}]+\}")
            )
...
class _IsoTool(QWidget):
    def __init__(self, win) -> None:
        ...
        self._mti = QLineEdit("0200")
        self._mti.setFixedWidth(64)
        self._de = QLineEdit()
        self._de.setPlaceholderText("DE (nº)")
        self._de.setFixedWidth(80)
        self._val = QLineEdit()
        self._val.setPlaceholderText("valor (hex o texto)")
        self._hex = QCheckBox("hex")
        self._hex.setChecked(True)
        setb = QPushButton("Set campo")
        setb.clicked.connect(self._set)
        delb = QPushButton("Quitar sel.")
        ...
        row2 = QHBoxLayout()
        row2.addWidget(build)
        row2.addWidget(self._host, 1)
        row2.addWidget(self._port)
    ...
    def _refresh(self) -> None:
        ...
        for i, de in enumerate(self._des):
            ...
            for j, val in enumerate(
                (str(de), spec.name if spec else "?", to_hex(self._fields[de]))
            ):
    ...
    def _set(self) -> None:
        try:
            ...
            val = (
                from_hex(self._val.text().strip())
                if self._hex.isChecked()
                else self._val.text().encode("latin-1")
            )
        except ValueError:
            self.win.notify.emit("DE o valor inválido.")
            return
        self._fields[de] = val
        self._de.clear()
        self._val.clear()
    ...
    def _del(self) -> None:
        ...
        if 0 <= row < len(self._des):
            self._fields.pop(self._des[row], None)
            self._refresh()
    ...
    def _build(self) -> None:
        try:
            ...
            self.win.notify.emit(f"No se pudo construir: {e}")
            return
        self._built = msg
    ...
    def _send(self) -> None:
        if not self._built:
            self.win.notify.emit("Construye el mensaje primero.")
            return
        port = self._port.text().strip()
        if not port.isdigit() or int(port) == 0:
            self.win.notify.emit("Indica un puerto válido.")
            return
        self._log.appendPlainText(f"→ enviando a {self._host.text()}:{port}…")
        self.win.send_iso8583(
            self._host.text().strip(), int(port), self._built, self._hdr.currentData()
        )
...
class _WriteTool(QWidget):
    def __init__(self, win) -> None:
        ...
        warn.setMaximumHeight(48)

        self._op = QComboBox()
        for label, val in _OPS:
            self._op.addItem(label, val)
        self._sfi = QLineEdit()
        self._sfi.setPlaceholderText("SFI")
        self._sfi.setFixedWidth(70)
        self._num = QLineEdit()
        self._num.setPlaceholderText("registro / offset")
        self._num.setFixedWidth(120)
        self._tag = QLineEdit()
        self._tag.setPlaceholderText("tag (PUT DATA)")
        self._tag.setFixedWidth(120)
        row1 = QHBoxLayout()
        for x in (self._op, self._sfi, self._num, self._tag):
            row1.addWidget(x)
        row1.addStretch(1)
        self._data = QLineEdit()
        self._data.setPlaceholderText("datos en hex")
        write = QPushButton("Escribir")
        write.clicked.connect(self._write)
        row2 = QHBoxLayout()
        row2.addWidget(self._data, 1)
        row2.addWidget(write)
    ...
    def _write(self) -> None:
        ...
        try:
            ...
            self.win.notify.emit("Datos hex inválidos.")
            return
        params = {"data": data}
        try:
            if op == "record":
                params["sfi"] = int(self._sfi.text())
                params["record"] = int(self._num.text())
            elif op == "binary":
                params["offset"] = int(self._num.text() or "0")
                sfi = self._sfi.text().strip()
            ...
            self.win.notify.emit("SFI/registro/offset/tag inválido.")
            return
    ...
    def log_result(self, op: str, resp) -> None:
        ...
        self._log.appendPlainText(
            f"{op.upper()}  SW {resp.sw_hex}  {cardwrite.write_status(resp.sw)}"
        )
        if resp.data:
