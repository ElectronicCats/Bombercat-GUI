...
class ProjectsPanel(QWidget):
    def __init__(self, win) -> None:
        ...
        self._name.setPlaceholderText("nombre del proyecto")
        self._desc = QLineEdit()
        self._desc.setPlaceholderText("descripción (opcional)")
        self._path = QLineEdit()
        self._path.setPlaceholderText(
            "ruta (opcional; vacío = ~/.local/share/emvy/projects)"
        )
        eng = QPushButton("engagements/")
        eng.clicked.connect(self._fill_eng)

        form1 = QHBoxLayout()
        form1.addWidget(self._name)
        form1.addWidget(self._desc)
        form2 = QHBoxLayout()
        form2.addWidget(self._path, 1)
        form2.addWidget(eng)

        new = QPushButton("Crear + activar")
        new.clicked.connect(self._new)
        use = QPushButton("Activar selección")
        use.clicked.connect(self._use)
        dele = QPushButton("Eliminar selección")
        dele.clicked.connect(self._del)
        actions = QHBoxLayout()
    ...
    def reload(self) -> None:
        ...
        for p in store.list_projects():
            ...
            rows.append((mark, p.name, p.created, p.description))
            self._rows.append((p.name, None))
        for p in store.list_path_projects():
            mark = "●" if active_rp and p.path.resolve() == active_rp else ""
            desc = f"{p.description}  ·  {p.path}" if p.description else str(p.path)
            rows.append((mark, p.name, "engagement", desc))
            self._rows.append((p.name, p.path))
