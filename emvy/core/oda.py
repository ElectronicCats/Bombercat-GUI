...
def oda_report(tlvs) -> OdaReport:
    ...
    notes: list[str] = []
    for label, bits in (("clave de CA (por cert emisor)", ca_bits),
                        ("clave del emisor (por cert ICC)", issuer_bits)):
        if bits is not None:
            if bits < 1024:
                notes.append(f"{label}: ~{bits} bits → DÉBIL (rota; esquemas exigen ≥1408)")
            elif bits < 1408:
                notes.append(f"{label}: ~{bits} bits → obsoleta/mínima")
    if "SDA" in methods and not dda_ready:
        notes.append("Solo SDA (sin DDA/CDA): los datos estáticos son clonables/replayables")

    ca_idx = find("8F")
    return OdaReport(
        methods_supported=methods, present=present,
        missing_for_sda=missing_sda, sda_ready=sda_ready, dda_ready=dda_ready,
        ca_index=(to_hex(ca_idx) if ca_idx else None),
        ca_key_bits=ca_bits, issuer_key_bits=issuer_bits, notes=notes,
    )
...
def recover_issuer_pk(cert90: bytes, exp9F32: bytes, ca_modulus: bytes,
                      ca_exponent: int, rem92: bytes = b"") -> IssuerPublicKey:
    ...
    if not cert90 or len(cert90) != n_ca:
        raise ValueError(f"cert (tag 90) de {len(cert90)}B no coincide con CA de {n_ca}B")
    ...
    return IssuerPublicKey(
        header_ok=header_ok, trailer_ok=trailer_ok, format_ok=format_ok,
        hash_ok=hash_ok, issuer_id=issuer_id, expiry=expiry, serial=serial,
        modulus=modulus, exponent=exp9F32, valid=valid,
    )


def verify_issuer_certificate(tlvs, ca_modulus: bytes,
                              ca_exponent: int) -> IssuerPublicKey | None:
