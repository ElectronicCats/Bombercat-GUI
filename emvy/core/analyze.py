...
class Assessment:
    ...
    def summary(self, color=None) -> str:
        ...
        if self.auc:
            ...
            L.append("  " + (", ".join(emvbits.set_flags(self.auc)) or "(sin restricciones)"))
        if self.ttq:
        ...
        L.append(f"  SDA listo: {'sí' if o.sda_ready else 'no'}   "
                 f"DDA listo: {'sí' if o.dda_ready else 'no'}")
        if o.ca_key_bits:
            L.append(f"  clave CA ~{o.ca_key_bits} bits · emisor ~{o.issuer_key_bits or '?'} bits")
        if self.issuer_key is not None:
            ik = self.issuer_key
            estado = "VÁLIDO" if ik.valid else "INVÁLIDO"
            L.append(color(f"  cert. emisor recuperado: {estado}", "bold")
...
def assess(tlvs, *, ca_modulus: bytes | None = None,
           ca_exponent: int = 3) -> Assessment:
    ...
    if issuer_key is not None and not issuer_key.valid:
        findings.append("El certificado de clave pública del emisor NO valida "
                        "con la CA indicada (hash/cabecera/formato)")
    return Assessment(aip=aip, auc=auc, ttq=ttq, cvm=cvm, oda=oda,
                      issuer_key=issuer_key, findings=findings)
